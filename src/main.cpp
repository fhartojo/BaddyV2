...
bool format_file_system_spifs()
{
    ...
    File file = SPIFFS.open(BADDY_CONFIG_FILENAME, "w"); // Open BADDY config file
...
int set_speed_spifs(String Json)
{
    ...
    File file = SPIFFS.open(BADDY_CONFIG_FILENAME, "r");
    ...
    else
    {
        ...
        else
        {
            ...
            File file = SPIFFS.open(BADDY_CONFIG_FILENAME, "w");
...
void ReadSequence()
{
    ...
    if (loop_mode_seq == 0)
    {
        ...
        motorStop();
        if (PlayMode == 2)
        { // Transmit to BUDDY if PlayMode 2
            buddy_send_abort();
        }
        printFrown();
        return;
